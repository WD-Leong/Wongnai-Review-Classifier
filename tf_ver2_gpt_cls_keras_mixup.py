...
class Decoder(tf.keras.layers.Layer):
    ...
    def mix_output(self, x1, x2, alpha, training=True):
...
class GPTClassifierDecoder(tf.keras.Model):
    def __init__(
        ...
        self.o_decoder = tf.keras.layers.Dense(n_classes)
        self.p_decoder = tf.keras.layers.Dense(vocab_size)
    
    def call(self, x, training=True):
        dec_outputs = self.gpt_model(x, training=training)
        avg_outputs = tf.reduce_mean(dec_outputs, axis=1)
        
        # Linear Probe for classification. #
        dec_logits = self.p_decoder(dec_outputs)
        cls_logits = self.o_decoder(avg_outputs)
        return cls_logits, dec_logits
    
    def mix_output(
        self, x1, x2, alpha_mix, training=True):
        dec_outputs = self.gpt_model.mix_output(
            x1, x2, alpha_mix, training=training)

        # Linear Probe for classification. #
        dec_logits = self.p_decoder(dec_outputs)
        cls_logits = self.o_decoder(
            tf.reduce_mean(dec_outputs, axis=1))
        return cls_logits, dec_logits
    ...
    def generate(self, x):
        ...
        for step in range(self.seq_len):
            ...
            tmp_logits = self.call(tmp_inputs, training=False)[1]
