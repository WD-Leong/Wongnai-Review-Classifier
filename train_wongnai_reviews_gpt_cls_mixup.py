...
import tf_ver2_gpt_cls_keras_mixup as gpt_cls
from sklearn.metrics import classification_report

# Function for the input generation. #
def generate_sample(
    token_input, word_2_idx, seq_length, EOS_token):
...
def sub_batch_train_step(
    model, sub_batch_sz, vocab_sz, n_class, 
    x_encode1, x_encode2, x_output1, x_output2, optimizer, 
    reg_cls=0.0, reg_lm=0.0, learning_rate=1.0e-3, grad_clip=1.0):
    ...
    for n_sub in range(sub_batch):
        ...
        tmp_decode1 = x_encode1[id_st:id_en, 1:]
        tmp_encode1 = x_encode1[id_st:id_en, :-1]

        tmp_output2 = x_output2[id_st:id_en]
        tmp_decode2 = x_encode2[id_st:id_en, 1:]
        tmp_encode2 = x_encode2[id_st:id_en, :-1]
        
        alpha = np.expand_dims(
            np.random.uniform(size=id_en-id_st), axis=1)
        tmp_output = tf.add(
            alpha * tf.one_hot(tmp_output1, n_class), 
            (1.0-alpha) * tf.one_hot(tmp_output2, n_class))
        
        alpha = np.expand_dims(alpha, axis=2)
        tmp_decode = tf.add(
            alpha * tf.one_hot(tmp_decode1, vocab_sz), 
            (1.0-alpha) * tf.one_hot(tmp_decode2, vocab_sz))
        
        with tf.GradientTape() as grad_tape:
            model_outputs = model.mix_output(
...
model_ckpt_dir  = "TF_Models/thai_reviews_gpt_cls_mixup_len_256"
train_loss_file = "train_loss_thai_reviews_gpt_cls_mixup_len_256.csv"
...
while n_iter < maximum_iter:
    ...
    for n_index in range(batch_size):
        ...
        tmp_i_tok1 = train_data[tmp_index1][1]

        tmp_index2 = batch_mixaug[n_index]
        tmp_label2 = train_data[tmp_index2][0]
        tmp_i_tok2 = train_data[tmp_index2][1]
        
        tmp_p_idx1 = generate_sample(
            tmp_i_tok1, word_2_idx, seq_length, EOS_token)
        tmp_p_idx2 = generate_sample(
            tmp_i_tok2, word_2_idx, seq_length, EOS_token)
        n_sw_toks1 = len(tmp_p_idx1)
        n_sw_toks2 = len(tmp_p_idx2)

        tmp_out_lab1[n_index] = tmp_label1
        tmp_out_lab2[n_index] = tmp_label2
        
        tmp_in_seq1[n_index, :n_sw_toks1] = tmp_p_idx1
        tmp_in_seq2[n_index, :n_sw_toks2] = tmp_p_idx2

    # Update the network weights. #
    tmp_loss = sub_batch_train_step(
        cls_model, sub_batch, vocab_size+4, num_class, 
        tmp_in_seq1, tmp_in_seq2, tmp_out_lab1, tmp_out_lab2, 
...
tmp_result_file = "thai_review_gpt_mixup_"
