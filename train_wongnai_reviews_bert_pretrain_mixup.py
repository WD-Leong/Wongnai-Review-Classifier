...
import tf_ver2_bert_keras_mixup as bert
...
def sub_batch_train_step(
    model, sub_batch_sz, 
    vocab_size, n_class, x_mask1, x_mask2, 
    ...
    for n_sub in range(sub_batch):
        ...
        tmp_label = tf.add(
            alpha*tf.one_hot(tmp_label1, n_class, axis=-1), 
            (1.0-alpha)*tf.one_hot(tmp_label2, n_class, axis=-1))
        
        alpha = np.expand_dims(alpha, axis=2)
        tmp_output = tf.add(
            alpha*tf.one_hot(tmp_output1, vocab_size, axis=-1), 
            (1.0-alpha)*tf.one_hot(tmp_output2, vocab_size, axis=-1))
        
        with tf.GradientTape() as grad_tape:
            model_outputs = model.mixup_aug(
...
model_ckpt_dir  = "TF_Models/thai_reviews_bert_mixup"
train_loss_file = "train_loss_thai_reviews_bert_mixup.csv"
...
tmp_in_mask1 = np.zeros(
    [batch_size, seq_length+2], dtype=np.float32)
...
while n_iter < maximum_iter:
    ...
    for n_index in range(batch_size):
        ...
        input_tuple1 = prepare_input(
            tmp_token1, word_2_idx, seq_length, vocab_size, 
            p_mask, CLS_token, TRU_token, EOS_token, MSK_token)
        
        input_tuple2 = prepare_input(
            tmp_token2, word_2_idx, seq_length, vocab_size, 
            p_mask, CLS_token, TRU_token, EOS_token, MSK_token)
        
        n_input1 = len(input_tuple1[1])
        n_input2 = len(input_tuple2[1])

        tmp_out_lab1[n_index] = tmp_label1
        tmp_out_lab2[n_index] = tmp_label2

        tmp_in_seq1[n_index, :n_input1]  = input_tuple1[1]
        tmp_in_seq2[n_index, :n_input2]  = input_tuple2[1]
        tmp_in_mask1[n_index, :n_input1] = input_tuple1[2]
        tmp_in_mask2[n_index, :n_input2] = input_tuple2[2]
        tmp_out_seq1[n_index, :n_input1] = input_tuple1[0]
        tmp_out_seq2[n_index, :n_input2] = input_tuple2[0]
    
    # Update the weights. #
    tmp_loss = sub_batch_train_step(
        bert_model, sub_batch, vocab_size+6, 
        num_class, tmp_in_mask1, tmp_in_mask2, 
        tmp_in_seq1, tmp_in_seq2, tmp_out_seq1, 
        tmp_out_seq2, tmp_out_lab1, tmp_out_lab2, 
...
tmp_result_file = "thai_review_bert_mixup_"
